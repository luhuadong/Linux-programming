...
/* convert char -> 6-bit value */
static BOOL b64_is_valid(char c)
{
	if ('A' <= c && c <= 'Z')
		return TRUE;
	else if ('a' <= c && c <= 'z')
...
int base64_decode(const char *in, unsigned int ilen, unsigned char *out, unsigned int *olen)
{
	const char *in_end = in + ilen;
	const unsigned char *o = out;
...
/*------------------------------------end of BASE64 file---------------------------------*/
static int TelnetTickCount(void)
{
#ifdef _WIN32
    return GetTickCount();
...
unsigned int GetTelentAuthKey(char *buf, unsigned int length)
{
    ...
    unsigned int keyLength, i, bufLen;
    md5_state_t md5;

    keyLength = TMP_BUF_SIZE;
    if ( keyLength > length * 3 / 4 ) {
        keyLength = length * 3 / 4;
    }

    for ( i = 0; i < keyLength / 16; ++i ) {
        int value = TelnetTickCount();

        md5_init(&md5);
        md5_append(&md5, (const unsigned char*)&value, 4);
        md5_finish(&md5, tmpBuf + i * 16);
    }

    if ( keyLength % 16 > 0 ) {
        int value = TelnetTickCount();
        int len = keyLength % 16;

        md5_init(&md5);
        md5_append(&md5, (const unsigned char*)&value, 4);
        md5_finish(&md5, tmpBuf + keyLength - len);
    }

    bufLen = length;
    base64_encode(tmpBuf, keyLength, buf, &bufLen);

    for ( i = bufLen; i < length; ++i ) {
        buf[bufLen] = b64_table[TelnetTickCount() % 64];
    }
...
{
#ifndef VXWORKS_OS
	return base64_decode(in,ilen,out,olen);
#else
	return 0;
#endif
}
