...
static ssize_t dodev_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
...
static ssize_t dodev_write(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    struct  dodev *pdo=NULL;
    int val;
    int ret=0;

    copy_from_user(&val,buf,sizeof(int));
...
static long dodev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        case DO_CONF_PLUSE_PERIOD :
        {
            ...
            int dev=pdo->chan+1;
            copy_from_user(&period,(unsigned short *)arg,sizeof(unsigned short));
...
static ssize_t didev_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    struct  didev *pdi=NULL;
    int val;
...
static long didev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        case DI_GET_PLUSE_COUNT :
        {
            ...
            copy_to_user((unsigned long *)arg,&counter,sizeof(unsigned long));
        }
        break;
...
static ssize_t aidev_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    struct  aidev *pai=NULL;
    float  val;
...
static long aidev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case AI_SET_VOLT_CURR_CAL :
        {
            ...
            copy_from_user(&ai_calValue,(char *)arg,sizeof(struct ai_calibra));
        ...
        case AI_GET_VOLT_CURR_CAL :
        {
            ...
            copy_to_user((char *)arg,&ai_calValue,sizeof(struct ai_calibra));
        }
        break;
        // new add
        case AI_SET_ANALOG_MODE :
        {
            ...
            copy_from_user(&analog_mode,(char *)arg,sizeof(int));
        ...
        case AI_GET_ANALOG_VALUE :
        {
            ...
            copy_from_user(&analog_value,(char *)arg,sizeof(struct ai_analog_value));
            ...
            copy_to_user((char *)arg,&analog_value,sizeof(struct ai_analog_value));
        }
        break;
            //
...
static ssize_t dev_reboot_write(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
