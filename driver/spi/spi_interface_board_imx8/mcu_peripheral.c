...
k37adev_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    case 1://ibutton code
        {
            ...
            {
                residue = copy_to_user(buf,&user_info,sizeof(struct user_info));
    ...
    case 2://k37a door
        {
            ...
            residue = copy_to_user(buf,&door_status,sizeof(door_status));
    ...
    case 3://k37a locker
       {
            ...
            residue = copy_to_user(buf,&locker_status,sizeof(locker_status));
    ...
    case 5://k37a get mcu log
        {
            char * log_buff = kzalloc(DATA_MAX_LENGTH, GFP_KERNEL);
            int log_len;
            char order;
            
            if(log_buff == NULL) {
                printk("kzalloc log_buff is fail \n");
                ret=-1; 
                goto K37ADEV_READ_OUT;
            }

            residue = copy_from_user(&order, buf, 1);
            ret = get_mcu_log(&order,log_buff, &log_len,1000,k37a_dev->pro);
            if(ret!=0){
                ret=-1; 
                goto K37ADEV_READ_OUT;
            }     
            residue = copy_to_user(buf,log_buff,log_len);
            kfree(log_buff);
            ret = log_len;                    
        }
        break;
    ...
    case 7://k37a internal temperature
        {
            ...
            residue = copy_to_user(buf,&k37a_temperature,sizeof(k37a_temperature));
    ...
    case 8://k37a battery
        {
            ...
            residue = copy_to_user(buf,&k37a_battery_quantity,sizeof(k37a_battery_quantity));
...
k37adev_write(struct file *filp,const char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    case 0://users information
        {
            ...
            residue = copy_from_user(&user_list, buf, sizeof(struct user_info_list));
    ...
    case 3://k37a locker
        {// 1=open 0=close
            ...
            residue = copy_from_user(&locker_status,buf,sizeof(locker_status));
    ...
    case 6://sync mcu time
        {
            long seconds=0;
            residue = copy_from_user(&seconds,buf,sizeof(long));
    ...
    case 9: //update
        {
            ...
            int recv_len;
            struct firmware_packet *one_packet = kzalloc(sizeof(struct transfer_result), GFP_KERNEL);

            if(one_packet == NULL) {
                printk("kzalloc firmware_packet is fail \n");
                ret= -1;
                goto K37ADEV_WRITE_OUT;
            }
            ...
            residue = copy_from_user(&one_packet->order, buf, count);
    ...
    case GET_K37A_POWER_SUPPLY:
        {
            ...
            residue = copy_to_user((char*)arg,&supply,sizeof(supply));
        }
        break;
    case GET_K37A_BATTERY_CHARGE:
        {
            ...
            residue = copy_to_user((char*)arg,&charge,sizeof(charge));         
        }
        break;            
    ...
    case BATTERY_GET_VOLT_VALUE :// source battery voltage value.
        {
            ...
            residue = copy_to_user((char *)arg,&bat_vol,sizeof(struct battery_voltage));
        }
        break;
    case BATTERY_SET_VOLT_CAL :
        {
            ...
            residue = copy_from_user(&bat_calValue,(char *)arg,sizeof(struct battery_calibrate));
    ...
    case BATTERY_GET_VOLT_CAL :
        {
            ...
            residue = copy_to_user((char *)arg,&bat_calValue,sizeof(struct battery_calibrate));
        }
        break;
    ...
    case MCU_MEMORY_TEST:
        {
            ...
            residue = copy_from_user(&mem_type,(char *)arg,sizeof(int));
    ...
    case GET_MCU_MEMORY_TEST_RESULT:
        {
            ...
            residue = copy_from_user(&mem_res,(char *)arg,sizeof(struct memory_result));
            ...
            residue = copy_to_user((char *)arg,&mem_res,sizeof(struct memory_result));
        }
        break;
