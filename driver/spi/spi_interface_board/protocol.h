...
static unsigned short get_recv_len(const char * const packet)
{
    unsigned short len=0;

    if(!packet) return -1;
    
    len = packet[RECV_DATA_POS-2]<<8;
    len += packet[RECV_DATA_POS+1-2];
    return len;
}
...
static unsigned short get_ctrller_len(const char * const packet)
{
    unsigned short len=0;

    if(!packet) return -1;
    
    len = packet[CTRLLER_DATA_POS-2]<<8;
    len += packet[CTRLLER_DATA_POS+1-2];
    return len;
}
...
static unsigned char get_cmdflag(const char * const packet)
{
    unsigned char gflag=0;

    if(!packet) return -1;
    
    gflag= CMD_FLAG(packet[CMD_POS]);
    return gflag;
